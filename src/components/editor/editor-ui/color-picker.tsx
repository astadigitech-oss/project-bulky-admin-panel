...
import { Button, ButtonProps as ButtonType } from "@/components/ui/button";
...
function createColorPickerStore(
  ...
  const store: ColorPickerStore = {
    ...
    setColor: (value: ColorValue) => {
      ...
      if (!current || Object.is(current.color, value)) return;

      // UPDATE IMUTABEL: Jangan mutasi properti, ganti objek state-nya
      stateRef.current = {
        ...current,
        color: value,
      };
      ...
      if (callbacks?.onColorChange) {
        callbacks.onColorChange(colorToString(value, current.format));
      }
      store.notify();
    },
    setHsv: (value: HSVColorValue) => {
      const current = stateRef.current;
      if (!current || Object.is(current.hsv, value)) return;
      ...
      if (callbacks?.onColorChange) {
        ...
        callbacks.onColorChange(colorToString(colorValue, current.format));
      }
      store.notify();
    },
    setOpen: (value: boolean) => {
      const current = stateRef.current;
      if (!current || current.open === value) return;

      stateRef.current = { ...current, open: value };
      callbacks?.onOpenChange?.(value);
      store.notify();
    },
    setFormat: (value: ColorFormat) => {
      const current = stateRef.current;
      if (!current || current.format === value) return;

      stateRef.current = { ...current, format: value };
...
const ColorPickerRoot = (props: ColorPickerRootProps) => {
  ...
  const storeCallbacks: ColorPickerStoreCallbacks = {
    onColorChange: onValueChange,
    onFormatChange,
    onOpenChange: (open) => {
      onOpenChange?.(open, undefined as any);
    },
  };

  const store = createColorPickerStore(listenersRef, stateRef, storeCallbacks);
...
function ColorPickerRootImpl(props: ColorPickerRootImplProps) {
  ...
  const onPopoverOpenChange = (newOpen: boolean) => {
    ...
    onOpenChange?.(newOpen, undefined as any);
  };

  const RootPrimitive = render ? render : "div";
...
// Definisikan tipe fungsi render yang spesifik
type RenderFn<TProps = any, TState = any> = (
  props: TProps,
  state: TState,
) => React.ReactElement;

// Tipe gabungan untuk render prop
type PolymorphicRender = React.ElementType | React.ReactElement | RenderFn;
...
function ColorPickerTrigger(props: ColorPickerTriggerProps) {
  ...
  const renderProp = (render || Button) as PolymorphicRender;

  return (
    <PopoverTrigger
      disabled={context.disabled}
      render={(popoverProps, state) => {
        const finalProps = {
          ...triggerProps,
          ...popoverProps,
        ...
        // 1. Jika renderProp adalah FUNGSI (Render Fn)
        if (typeof renderProp === "function") {
          // Kita asumsikan ini adalah RenderFn dari Base UI
          const renderFn = renderProp as RenderFn;
          return renderFn(finalProps, state);
        }

        // 2. Jika renderProp adalah ELEMEN (JSX yang sudah jadi)
        if (React.isValidElement(renderProp)) {
          return React.cloneElement(renderProp, {
            ...finalProps,
            className: cn(
              (finalProps as any).className,
              (renderProp.props as any).className,
            ),
          } as any);
        }

        // 3. Jika renderProp adalah Komponen atau String (div, button, dll)
        const Tag = renderProp as React.ElementType;
        return <Tag {...(finalProps as any)} />;
      }}
    />
  );
}
...
function ColorPickerContent(props: ColorPickerContentProps) {
  const { render, className, children, ...popoverContentProps } = props;
  const context = useColorPickerContext("ColorPickerContent");

  if (context.inline) {
    // 1. Tentukan fallback jika render tidak ada
    const renderProp = render || "div";

    // 2. Jika render adalah Fungsi (ComponentRenderFn)
    if (typeof renderProp === "function") {
      return renderProp(
        {
          ...popoverContentProps,
          "data-slot": "color-picker-content", // TS sekarang akan mengizinkan ini
    ...
    // 3. Jika render adalah Elemen yang sudah jadi (ReactElement)
    if (React.isValidElement(renderProp)) {
      ...
      // Casting di atas memberitahu TS bahwa atribut data-slot itu valid.
    }

    // 4. Jika render adalah Tipe Komponen (string "div" atau Komponen lain)
...
function ColorPickerArea(props: ColorPickerAreaProps) {
  ...
  const AreaPrimitive = render ? render : "div";
...
interface ColorPickerHueSliderProps extends React.ComponentProps<
  typeof SliderPrimitive.SliderRoot
> {}
  ...
  return (
    ...
    >
      <SliderPrimitive.Slider.Control>
        <SliderPrimitive.Slider.Track className="relative h-3 w-full grow overflow-hidden rounded-full bg-[linear-gradient(to_right,#ff0000_0%,#ffff00_16.66%,#00ff00_33.33%,#00ffff_50%,#0000ff_66.66%,#ff00ff_83.33%,#ff0000_100%)]">
          <SliderPrimitive.Slider.Indicator className="absolute h-full" />
          <SliderPrimitive.Slider.Thumb className="border-primary/50 bg-background focus-visible:ring-ring block size-4 rounded-full border shadow transition-colors focus-visible:ring-1 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50" />
...
function ColorPickerSwatch(props: ColorPickerSwatchProps) {
  const { render, className, ...swatchProps } = props;
  ...
  const SwatchPrimitive = render ? render : "div";
...
function ColorPickerEyeDropper(props: ButtonType) {
...
function ColorPickerFormatSelect(props: ColorPickerFormatSelectProps) {
  ...
  return (
    <Select
      ...
      {...selectProps}
      value={format}
      onValueChange={(v) => onFormatChange(v as "hex" | "rgb" | "hsl" | "hsb")}
      disabled={context.disabled}
    >
      <SelectTrigger
