...
import { VideoCategoryType } from "../_api/types";
...
export const column = ({
  ...
  },
  {
    ...
    header: "Status",
    cell: ({ row }) => {
      if (typeof row.original.is_active !== "boolean") {
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className={"w-auto"}>
            <DropdownMenuGroup>
              ...
              {typeof row.original.is_active === "boolean" && (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
                    className={"text-xs"}
                    onClick={() =>
                      handleChangeStatus(
                        row.original.is_active
                          ? `Nonaktifkan ${row.original.nama_id}`
                          : `Aktifkan ${row.original.nama_id}`,
                        row.original.id,
              ...
              <DropdownMenuItem
                ...
                onClick={() => {
                  setOpen("edit");
                  setQuery({ videoCategoryId: row.original.id });
                }}
              >
              ...
              <DropdownMenuItem
                className={"text-xs"}
                onClick={() =>
                  handleDelete(row.original.nama_id, row.original.id)
                }
                variant="destructive"
              >
