...
import { useCreateTagBlog, useUpdateTagBlog } from "../../_api";
...
export const DialogFormTagBlog = ({
  ...
  detail?: TagBlogType & TagBlogPartIType;
  isDisabled?: boolean;
}) => {
  const idFormTagBlog = useId();
  ...
  const { mutate: createTagBlog, isPending: isCreating } = useCreateTagBlog();
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    switch (mode) {
      case "create":
        createTagBlog({ body: values }, { onSuccess: () => handleClose() });
        break;
      case "edit":
        updateTagBlog(
