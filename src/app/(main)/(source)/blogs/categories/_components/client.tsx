...
import {
  ...
  useGetBlogCategoryDetail,
  useGetBlogCategoryList,
  useGetBlogCategorySelect,
  useReorderBlogCategory,
  useToggleStatusBlogCategory,
} from "../_api";
import { DialogFormBlogCategory } from "./_dialog/form";

export const BlogCategoriesClient = () => {
  const [open, setOpen] = useState<"edit" | "create" | null>(null);
  const [{ sort, order, blogCategoryId }, setQuery] = useQueryStates(
    {
      blogCategoryId: parseAsString.withDefault(""),
      sort: parseAsString.withDefault("updated_at"),
      order: parseAsString.withDefault("desc"),
    },
    { urlKeys: { blogCategoryId: "id" } },
  );
  ...
  const { data: detail } = useGetBlogCategoryDetail({
    id: blogCategoryId,
  });
  const { data: selectData } = useGetBlogCategorySelect();

  const totalSelect = Array.isArray(selectData?.data)
    ? selectData?.data.length
    : ((selectData?.data as any)?.kategori?.length ?? 0);
  ...
  return (
    <div className="flex flex-col gap-6 pt-4">
      <DialogStatus />
      <DialogDelete />
      <DialogFormBlogCategory
        open={!!open}
        onOpenChange={(e) => {
          if (!e) {
            setOpen(null);
            if (blogCategoryId) setQuery({ blogCategoryId: "" });
          }
        }}
        mode={open}
        detail={detail?.data}
      ...
      <div className="flex items-center justify-between">
        <h1 className="leading-none font-semibold text-2xl">Kategori Berita</h1>
