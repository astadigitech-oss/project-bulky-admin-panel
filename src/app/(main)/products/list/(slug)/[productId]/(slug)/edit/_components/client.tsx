...
import { ChevronRight, Edit, Eye, Package, Send, XIcon } from "lucide-react";
...
import { useCreateProduct, useGetProductDetail } from "@api/product/list";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
export const ProductIdClient = () => {
  ...
  const { data: productDetail } = useGetProductDetail({ id: productId });
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    values: {
      ...
      kategori_id: detail?.kategori.id ?? "",
      kondisi_id: detail?.kondisi.id ?? "",
      kondisi_paket_id: detail?.kondisi_paket.id ?? "",
