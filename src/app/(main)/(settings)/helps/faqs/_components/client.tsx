...
import { DialogFormFAQs } from "./_dialog/form";

export const FAQHelpClient = () => {
  const [open, setOpen] = useState<"edit" | "create" | null>(null);
  const [{ sort, order, faqId }, setQuery] = useQueryStates(
    {
  ...
  const { mutate: deleteFAQs, isPending: isDeleting } = useDeleteFAQs();
  const { mutate: updateStatus, isPending: isUpdatingStatus } =
    useChangeStatusFAQs();
  const { mutate: reorderStatus, isPending: isReordering } = useReorderFAQs();
  ...
  const { data: detail } = useGetFAQsDetail({
    id: faqId,
  });
  ...
  const handleDelete = async (user: string, id: string) => {
    const ok = await confirmDelete(user, "faq");
    if (!ok) return;
    deleteFAQs({ params: { id } });
  };
  ...
  return (
    <div className="flex flex-col gap-6 pt-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="leading-none font-semibold text-2xl">FAQ&apos;s</h1>
